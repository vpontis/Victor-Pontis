...
var Conway_Game = function(grid, initial_squares){
	...
	if (initial_squares === undefined || initial_squares.length !== NUM_X || initial_squares[0].length !== NUM_Y){
	...
	var square_lives =  function(x_index, y_index, squares) {
		...
		square_neighbors(function(i,j){
				if ((i != 0 || j != 0) &&
					x_index + i >= 0 && x_index + i < NUM_X &&
					y_index + j >= 0 && y_index + j < NUM_Y) {
